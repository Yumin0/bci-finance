...
#### [一鍵複製分享連結給主管審核]（優先序2 列22，Layla 回饋；Q22c 落點提案待 Yumin 拍板）
- **已拍板**：按鈕放**職員的詳細頁**（職員複製連結分享給主管）；未登入點連結導 /login 後**自動跳回該單**（需確認/補 `proxy.ts` returnUrl 回跳）；雙身份主管不需優先邏輯（`current_step` 本來就決定審哪一關）
- **待拍板（Q22c）**：無權限者落點——提案做中介轉址路由（審核人→審核頁／申請人→自己單子／可查閱者→唯讀／皆非→「無檢視權限」提示頁），見 `docs/優化項目理解與提問_優先序2_2026-07-19.md` 列 22
- **範圍**：先做資金分配＋付款憑單（Layla 情境），沖銷是否要做待確認

#### [付款憑單列表加「申請金額」欄]（優先序2 列24，Cindy 回饋，2026-07-19 Yumin 拍板）
- **規格**：9 欄共用元件 `PaymentListCells` 在「核准金額」前加「申請金額」（＝憑單 `amount`），方便對照申請與核准的金額出入；**所有 9 欄列表一起加**（我的付款憑單／全部付款紀錄／首頁分頁／審核管理全部 Tab／財務付款憑單管理頁）
- **備註**：與列 26（拿掉發票憑證欄）動同一個共用元件，**兩項同一批做**；沖銷審核管理列表的「核准金額（母憑單）｜沖銷金額」欄位語意不同，是否比照加欄開工時再確認
- 詳見 `docs/優化項目理解與提問_優先序2_2026-07-19.md` 列 24

#### [列表拿掉發票憑證欄（只留內層顯示）]（優先序2 列26，2026-07-19 與財務共識）
- **規格**：外層列表不需要看附檔內容，**所有「列表」上的發票憑證欄全部拿掉**，內層（詳細頁附件欄位）照舊；CSV 匯出欄位不動
- **影響範圍（橫切關注點，完成時逐項確認）**：
  - [ ] 共用元件 `PaymentListCells` 移除發票憑證欄（我的付款憑單／全部付款紀錄／首頁付款憑單分頁連動）
  - [ ] 付款憑單審核管理全部 Tab（AccountGroupedList／HistoryList）
  - [ ] 財務付款憑單管理頁（/finance/payment）
  - [ ] 沖銷審核管理列表（發票憑證＝沖銷附件欄，一併拿掉）
  - [ ] 附件批次撈取（`getAttachmentsByPaymentIds` 等）若只剩列表在用可一併移除呼叫，減少查詢
- 詳見 `docs/優化項目理解與提問_優先序2_2026-07-19.md` 列 26
