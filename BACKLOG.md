...
#### ~~[核准金額下修通知申請人]~~（優先序2 列20）✅ 已完成（2026-07-20，見「已完成」）
...
#### ~~[列表拿掉發票憑證欄（只留內層顯示）]~~（優先序2 列26）✅ 已完成（2026-07-20，見「已完成」）

#### [付款憑單付款明細每組各自的「費用項目（細項）」欄位]（2026-07-19 Yumin 提出，開工前需先確認設計）
- **背景**：付款憑單的「費用項目」目前是整張一個、放在付款明細群組外（建立時繼承母單），明細每組只有 摘要｜稅額選擇｜未稅金額｜稅額｜總額。若一張憑單要付兩個不同費用項目，現況無法逐組標示。
- **表面上只是表單設定調整**（到表單設定把細項欄位加進付款憑單的付款明細群組、dataSource 選費用項目細項類別），**但會打破「費用項目整張一個」的現行前提**，至少影響：
  - 列表「費用項目」欄（9 欄共用元件 `PaymentListCells`，讀 `funds_payment.expense_item` 單值）——多組不同細項時該顯示哪個？
  - 付款憑單列印頁（`my-payment/[id]/print`）逐組明細的「費用項目（細項）」目前是**以摘要對回申請單帶入組**推出來的——群組內有真欄位後應改讀自己的欄位（反而變簡單）
  - 建立頁逐組帶入邏輯（申請單群組有細項、憑單群組也有細項後，label 對應會自動帶，需驗證）
  - `funds_payment.expense_item` 結構化欄位的存值邏輯（目前整張一值）
- **開工前需 Yumin／財務確認**：①真的有「一張憑單付多個費用項目」的實務需求，還是維持「不同費用項目就開不同憑單」（現行拆單邏輯）？②列表與財務報表上多細項憑單怎麼顯示？
...
開始：2026-07-20
規格（2026-07-19 拍板）：判斷欄位＝單據種類（申請單頂層欄位），選「國內公司」時付款明細不顯示手續費/稅額選擇/稅額三欄（未填預設 0），選國內個人/國外公司/國外個人才顯示；範圍先做資金分配（付款憑單 Q6c 未拍板不動）
做法：付款明細群組欄位接上既有 showWhen 條件顯示機制（新共用純函式 `lib/groupSlotVisibility.ts`，表單設定驅動、程式不寫死「國內公司」），隱藏欄位在彙總/合計/憑單金額計算與存檔一律視為 0（存空值），彙總列的手續費/稅額字樣也一併隱藏；表單設定已在 dev 把三欄設為「單據種類＝國內個人/國外公司/國外個人才顯示」（正式機需手動同步，見 prod-pending-sql）
影響範圍確認：
- [x] /funds-allocation/my-funds/add（新增，AddFundsForm）
- [x] /funds-allocation/my-funds/edit/[id]（編輯/詳細，含申請人檢視，EditFundsForm）
- [x] /funds-allocation/review/check/[id]（審核頁：審核人編輯表單走 EditFundsForm＋唯讀 FundsAllocationDetail 同步過濾）
- [x] 表單設定：三欄設 showWhen（dev 已設／正式機待同步已登記）
實測（Playwright，dev DB，2026-07-20）：新增頁未選單據種類→三欄隱藏 ✓；選國外公司→三欄出現，填費用1000＋手續費50＋稅額選擇（稅額自動算250）→彙總 總額1,300 ✓；切回國內公司→三欄消失、彙總即時變 費用1,000/總額1,000 ✓；存草稿→DB `amount=1000`、明細組手續費/稅額選擇/稅額存空值（填過的值不外漏）✓；草稿編輯頁重開三欄同樣隱藏 ✓；測試草稿已刪除。

**匯出付款憑單按鈕提早顯示（課處長關卡核准後即可匯出）**（Yumin，財務確認） ✅ 已完成（2026-07-20）
分支：`feature/yumin-print-before-payment`
開始：2026-07-20
說明：與財務確認，付款憑單走完課長/處長（org_role）關卡、輪到第一個「審核群組」步驟（如第三處支出課）起即可匯出列印版，不必等已付款——讓職員先印紙本跑後續簽核，未審關卡簽核欄人名留白。開放條件抽共用純函式 `lib/paymentPrintEligibility.ts` 的 `canExportPaymentVoucher`（approved/paid 一律可、pending 看 `current_step` ≥ 範本第一個 approval_group 步驟；不寫死步驟名稱，範本改名/加關卡不用改程式；範本無群組步驟時退回核准後才開放），三個使用點同步：憑單詳細頁「匯出付款憑單」按鈕、列印頁防線（直接打網址也擋）、財務付款憑單管理頁「匯出」欄。**無資料庫結構變更、無正式機待執行 SQL。**
實測（Playwright，dev DB，2026-07-20）：憑單86（pending 第1關課長）詳細頁無按鈕、列印頁顯示擋下訊息 ✓；憑單85（pending 第3關第三處支出課）詳細頁有按鈕、列印頁正常渲染 ✓；憑單68（已付款）照舊有按鈕 ✓；/finance/payment 匯出欄 85有/86無/68有 ✓。純讀取驗證、未寫入測試資料。
...
開始：2026-07-20
說明：資金分配審核核准時，若核准金額低於承接值（上一關核准金額／第一關申請金額），對申請人多發一則「核准金額調整」通知，body＝「『{關卡名}』將核准金額由 NT$X 調整為 NT$Y」；**每一關下修就通知一次；只做資金分配**（付款憑單不做，2026-07-19 Yumin 拍板）。實作在 `submitApprovalDecision`（`actions/approval-flow.ts`）：update 覆寫 `approved_amount` **之前**先讀承接值（`allocPrevCap`），通知區塊比對 `approvedAmount < allocPrevCap` 才發；走既有 `notifyApplicant`、type 沿用 `approved`（鈴鐺圖示 ✅），**不動資料庫**；審核頁／快速核准／批次核准三個入口共用同一個 server action，一處實作全數涵蓋。
實測（Playwright，dev DB，2026-07-20）：測試單申請 10,000 → 課長照承接值核 10,000（未下修）**沒有**發調整通知；處長下修核 8,000 → 申請人收到「核准金額調整 - {項目名}」通知（含「處長審核」與 10,000→8,000），`approved_amount` 正確寫 8,000；驗收截圖在桌面（審核頁下修＋申請人通知鈴兩張）；測試資料已清除。無資料庫結構變更、無正式機待執行 SQL。
...
開始：2026-07-20
說明：共用元件 `PaymentListCells` 在「核准金額」前加「申請金額」欄（＝憑單 `amount`，Cindy 回饋：對照申請與核准的金額出入），並把所有列表的「發票憑證」欄拿掉（與財務共識：外層列表不看附檔，附件只留內層詳細頁附件欄位；CSV 匯出欄位不動）。**沖銷審核管理列表不加申請金額欄**（2026-07-20 Yumin 拍板：該列表核准金額是母憑單的、沖銷金額是本單的，語意不同，再插申請金額反而混淆；只拿掉發票憑證欄）。附帶清理：批次撈附件的 `getAttachmentsByPaymentIds`／`getAttachmentsByTempVoucherIds` 已無呼叫端、從 `actions/attachments.ts` 移除（列表頁少一次附件查詢）。無資料庫結構變更。
影響範圍確認（Playwright 實測表頭，localhost:3000，2026-07-20）：
- [x] 共用元件 `PaymentListCells`（加申請金額欄、移除發票憑證欄）
- [x] 我的付款憑單（MyPaymentTableView）
- [x] 全部付款紀錄（AllPaymentTableView）
- [x] 首頁「付款憑單申請單」分頁（HomeTabView）
- [x] 付款憑單審核管理全部 Tab（PaymentReviewClient：AccountGroupedList／HistoryList）
- [x] 財務付款憑單管理頁（/finance/payment，欄序：9 欄｜付款分類｜付款執行｜匯出）
- [x] 沖銷審核管理列表（VoucherReviewClient，只拿掉發票憑證欄、無申請金額欄）
- [x] 附件批次撈取呼叫端清理（page.tsx／my-payment／all／funds-payment review／funds-voucher review 五處呼叫與兩個 action 皆移除）
- [x] npm run build 通過（零 error）；實測畫面確認申請金額有值（申請 100／核准 50 對照情境正確）
...
規格文件：[`docs/core-logic/優化第二批規格提案_2026-07-19.md`](docs/core-logic/優化第二批規格提案_2026-07-19.md) 第一節
說明：資金分配審核頁「審核進度」的核准金額改為逐組核准表格（費用項目細項｜摘要｜申請費用｜核准費用可改｜手續費原值｜稅額｜小計），稅額依各組稅額選擇自動重算，總核准金額＝各組小計自動加總不可手改；每關承接上一關逐項值（第一關帶申請值），已完成關卡收合式逐項明細檢視；快速/批次核准由 server 端自動承接上一關逐項值（呼叫端不用改）。逐項值存 `approval_records.approved_items jsonb`（`approved_amount` 照舊存總額）。轉付款憑單建立頁改讀最新一關 `approved_items` 逐組帶入未稅/稅額/總額（解 Eric 金額同步問題），被砍到 0 的組不帶入；舊單無逐項資料照舊退回原邏輯。共用純函式 `lib/approvedItems.ts`。
影響範圍確認：
- [x] /funds-allocation/review/check/[id]（審核頁逐項核准表格，Playwright 實測：承接申請值、改核准費用稅額即時重算、總額自動加總不可手改、存檔/下一關承接/收合明細）
...
- [x] /funds-payment/my-payment/add/[id]（轉憑單逐組帶入最新一關核准值，Playwright 實測兩組帶入正確；組數對不上/舊單退回原邏輯）
- [x] SQL 已執行：dev（2026-07-19）＋正式機（2026-07-19，合併 main 前先跑）
- [x] npm run build 全站編譯通過（零 error 零 warning）
