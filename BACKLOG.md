...
#### [帳號管理 + 角色功能權限整合重構]（Riku）
分支：`feature/riku-role-restructure`
開始：2026-06-17
- [ ] 把現有 8 個系統角色精簡成 4 個起始預設（一般職員、主管、財務人員、系統管理員），可自由新增/刪除/編輯
- [ ] 移除「對應組織職稱」欄位（舊系統遺留，現已造成兩塊互相干擾）
- [ ] 帳號管理 + 角色功能權限合成一頁，用 Tab 切換（Tab1：角色管理；Tab2：帳號列表）
...
- **需求**：像諮詢議會這種由多人共同審核的步驟，需要多個帳號都持有「諮詢議會成員」這個系統角色，每個人各自登入才能看到待審單
- **解法**：新增 `user_system_roles` 中介表（一個帳號對應多個系統角色），審核人比對邏輯改為「此人持有的所有系統角色裡，有沒有一個符合當前步驟要求」；帳號管理頁改為可複選系統角色
- **影響範圍**：資料庫（新增表）、帳號管理頁（UI）、審核邏輯（`getReviewerInfo`）
