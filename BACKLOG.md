...
- **待補列表**：
  ...
  - ~~付款憑單列表（`/funds-payment/all`）~~ ✅ 已完成（2026-05-25，Riku）
  - ~~全部暫付款沖銷憑單（`/funds-voucher/all`）~~ ✅ 已完成（2026-05-25，Riku）
  - 問題回報列表
  - 帳號管理列表
- **功能點**：
...
表單設定新增「附件上傳」欄位類型，管理員可將附件欄位放在任意位置；資金分配申請新增/編輯頁支援上傳 PDF、JPG、PNG，點擊以 Modal 預覽（圖片用 img、PDF 用 iframe）；審核頁顯示附件清單；付款憑單繼承申請單附件（唯讀）並可補傳本憑單附件；新增 `fund_attachments` 資料表、`fund-attachments` Storage Bucket、`AttachmentUpload` / `AttachmentPreviewModal` 共用組件、`attachments.ts` Server Actions
