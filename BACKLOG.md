...
**憑單欄位帶入補強＋暫付款沖銷單號＋採購單號撞號修正**（Yumin）
分支：`feature/yumin-voucher-serial-inherit`
開始：2026-07-14
說明（2026-07-14 Yumin 拍板）：
1. 付款憑單建立時「單據種類」「幣別」從申請單付款明細**第一組**帶入（可修改）
2. 暫付款沖銷憑單建立時自動帶入「採購單號」（＝原付款憑單採購單號，唯讀）與「申請日期」（＝建立當天，可改）
3. 新增**暫付款沖銷單號**：格式＝母付款憑單採購單號＋3 碼流水；`temp_vouchers` 加單號欄位（SQL 需登記 prod-pending-sql.md），沖銷列表（我的／全部／審核）顯示單號欄
4. 修正**付款憑單採購單號流水碼寫死 001 撞號 bug**：改為同母單遞增（001、002…），既有撞號舊單以 SQL 依建單順序補正（dev＋正式機）
5. 日期原則：資金分配申請日期維持只能選週三；付款憑單與沖銷憑單的日期改為**建立當天**（不再繼承母單申請日期）
影響範圍確認（單號與日期為橫切關注點）：
- [x] 付款憑單建立/草稿編輯頁（帶入、日期；建立頁採購單號預覽改為實際將編的號）
- [x] 付款憑單各列表與審核頁（讀取儲存的採購單號，補正 SQL 後即正確）
- [x] 付款憑單審核管理週次篩選（日期改建單日後依實際建單週分組，Yumin 已知悉）
- [x] 沖銷憑單建立頁（採購單號唯讀帶入/申請日期預設當天）
- [x] 沖銷憑單我的列表/全部紀錄/審核管理（待審＋歷史）/詳細頁/審核頁/首頁分頁/CSV 匯出（新增單號欄，grep temp_vouchers 全使用點確認）
- [x] docs/prod-pending-sql.md 登記（temp_vouchers 單號欄＋付款憑單舊採購單號補正＋沖銷舊單補號）
- [x] 核心邏輯文件第二節（沖銷單號規則＋日期原則＋撞號修正）與第九節（帶入標記、9-5 勾銷）同步更新
- [ ] dev Supabase 執行 SQL（等 Yumin 執行）
- [ ] staging 測試
...
- [ ] 財務付款憑單管理頁（/finance/payment）＋審核管理群組 Tab 的「付款分類／付款執行」欄與帳戶標頭批次操作 —— **第二批待做**（等筑今第三課支出課／第三處處長截圖，見 BC 核心邏輯文件第六節）
