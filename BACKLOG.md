...
影響範圍確認（單號與日期為橫切關注點）：
- [ ] 付款憑單建立/草稿編輯頁（帶入、日期）
- [ ] 付款憑單各列表與審核頁（採購單號流水修正後顯示）
- [ ] 付款憑單審核管理週次篩選（日期改建單日後分週依據改變）
- [ ] 沖銷憑單建立頁（採購單號/申請日期帶入）
- [ ] 沖銷憑單我的列表/全部紀錄/審核管理/詳細頁（新增單號欄）
- [ ] docs/prod-pending-sql.md 登記（temp_vouchers 單號欄＋舊單號補正）
- [ ] 核心邏輯文件第二節（沖銷單號規則）與第九節（帶入標記）同步更新
