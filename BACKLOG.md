...
**暫付款沖銷憑單改版：多組明細繼承＋一單一沖銷＋核心邏輯文件重整**（Yumin）
分支：`feature/yumin-tempvoucher-groups`
開始：2026-07-14
說明（Yumin 拍板）：
1. 沖銷表單「會計科目」直接隱藏（自表單設定移除）
2. 付款明細改成**多組**（同付款憑單整組重複），逐組繼承付款憑單的「摘要用途、未稅金額、稅額」可修改，「總額」自填；沖銷金額＝各組總額加總（>0、≤原憑單實付金額）；`temp_vouchers` 加 `extra_data jsonb` 讓自訂欄位真正存檔（解 9-5 待補 2）
3. **一張付款憑單只能建一張沖銷憑單**：存在草稿/審核中/已核准的沖銷單就不顯示建立按鈕（退回的不算、可重建），存檔時後端也擋（兩道門）
4. 核心邏輯文件重整：新增「五、暫付款沖銷憑單列表」、後續章節順延；第八節防呆盤點整併成單一「規則｜目前狀況」表
影響範圍確認：
- [x] 沖銷建立頁（群組 UI＋逐組繼承摘要/未稅/稅額、總額自填、合計即時顯示）
- [x] 沖銷詳細頁/審核頁（有群組資料以表格逐組顯示、自訂欄位讀 extra_data）
- [x] 付款憑單詳細頁（一單一沖銷：按鈕隱藏、改顯示既有沖銷單連結）
- [x] createTempVoucher（extra_data 儲存、金額＝各組總額加總驗證、一單一沖銷後端擋）
- [x] dev 表單設定已改（移除會計科目＋群組標記＋總額必填）；正式機表單同步與 extra_data SQL 已登記 prod-pending-sql.md
- [x] 核心邏輯文件重整完成（v7：三/四/五列表、六對齊狀態、八規則總表、九總表更新）＋CLAUDE.md 同步
- [ ] dev Supabase 執行 extra_data SQL（等 Yumin）
- [ ] staging 測試
