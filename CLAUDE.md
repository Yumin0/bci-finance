...
| 財務管理 | `/finance` | 資金管理與付款憑單總覽；資金管理頁時間篩選改用共用自訂下拉元件（`WeekPicker`）；**付款憑單管理頁週次篩選（2026-07-19）**：/finance/payment 加年份＋週次下拉（共用 `WeekFilterBar`，含全部週次），依憑單建單日期前端過濾、預設當週，帳戶區塊金額彙總隨過濾清單計算；每列尾端「匯出」欄（僅已付款）連到付款憑單列印頁，財務可代任何職員輸出 |
