...
| 登入 | `/login` | Email + Password；Google OAuth（保留 Email 登入，同 email 自動合併帳號）；**returnUrl 登入後回跳（2026-07-20，列22）**：proxy 攔截未登入時把原路徑帶進 `/login?returnUrl=`，Email 登入（hidden input→`login` action）與 Google OAuth（`?returnUrl=` → 短效 cookie `google_oauth_return` → callback 讀回）登入成功後都跳回原頁；只接受站內相對路徑（`lib/returnUrl.ts` 的 `safeReturnUrl` 擋 open redirect），已登入者開 `/login?returnUrl=` 也直接轉去該頁 |
| 首頁 | `/` | 顯示我的申請紀錄與付款憑單 |
...
- `approval_records`：每筆審核動作記錄（取代舊 step1~5_* 欄位）；`approved_amount numeric` 儲存該步驟審核人填入的核准金額（僅 decision=approved 時有值）；`approved_items jsonb`（2026-07-19）儲存資金分配逐項核准明細 `[{ index, approved_base, other_fees, tax_amount }]`（組序／核准費用／手續費等其他數字欄原值合計／重算稅額，僅資金分配 decision=approved 時有值；`approved_amount`＝各組小計加總；insert 時僅有值才帶此欄位，避免正式機未加欄時其他單據審核跟著失敗）；`payment_category text` 儲存財務審核付款憑單/沖銷憑單（審核群組步驟）時加註的付款分類（僅 decision=approved 時有值，選項來自 `dropdown_options` field='payment_category'，後面關卡預設承接前面關卡最新選值、可調整，列表顯示最新一筆）
- `allocation_edit_logs`：欄位級變更歷程（變更人/時間/步驟/欄位/舊值/新值），原為資金分配審核頁編輯專用；2026-07-19 通用化為三種單據共用一張表（比照 `approval_records` 的三 FK 設計）：`funds_allocation_id` 放寬可空，新增 `funds_payment_id`／`temp_voucher_id` 可空關聯欄（temp_voucher_id 自 2026-07-20 起由沖銷審核頁編輯使用），擇一有值；`actions/edit-logs.ts` 的 `logFieldChanges`/`getEditLogs` 改收 target 物件（`fundsAllocationId`/`fundsPaymentId`/`tempVoucherId`），insert 失敗（如正式機未加欄）不擋儲存主流程只記 console；加欄 SQL 見 prod-pending-sql
...
- `fund_attachments`：資金申請/付款憑單附件（`funds_allocation_id`、`funds_payment_id`、`slot_label`、`storage_path`、`file_type`）；對應 Storage Bucket `fund-attachments`（Private）；上傳走 `/api/upload-attachment`，預覽走 `/api/attachment?path=...`（驗證登入後代理回傳，不暴露公開 URL）。**附件一律歸屬於表單的附件欄位（`form_slots` type='attachment'），`slot_label`＝欄位 label**，不再有頁面底部寫死的獨立「附件」區塊（2026-07-16 移除）；填寫頁/明細頁/審核頁都在各自的附件欄位內顯示該欄位的附件。**上游單據附件帶入「第一個附件欄位」內**（唯讀灰底、不可刪，與職員本張要補傳的檔在同一格，只有一個地方能上傳）：付款憑單帶資金分配申請單的附件（標「來自申請單」）、暫付款沖銷帶**兩層**——申請單附件標「來自申請單」＋母付款憑單附件標「來自付款憑單」（`getVoucherInheritedAttachments` 一次撈兩層，建立/詳細/審核三頁皆帶，讓沖銷時看得到預支當時傳的所有單據；`AttachmentItem.tag` 支援同一格逐檔標不同來源）。**沖銷詳細頁草稿狀態的附件欄位可直接補傳/刪除**（上傳即存檔，該頁無另外的儲存鈕；送審後轉唯讀）。附件欄位與 `slot_label` 的對應規則抽為 `lib/attachmentSlots.ts`（`attachmentSlotLabels` / `firstAttachmentSlotLabel` / `attachmentsForSlot`）：**`slot_label` 對不上任何附件欄位的舊檔（例：舊付款憑單底部區塊時代寫死的 `slot_label='payment'`、或表單事後改過欄位名稱）一律收進第一個附件欄位**，否則檔案還在資料庫、畫面卻整個消失。唯讀顯示共用 `RecordDetailView` 的 `DetailAttachmentField`（三個明細/審核頁一致，確保審核人在任何頁都看得到職員上傳的單據——2026-07-16 前 `FundsPaymentDetail`／沖銷詳細頁／沖銷審核頁都把附件欄位當文字框渲染，審核人看不到任何附件）
- `form_schema_rows` / `form_slots`：動態表單配置；`form_schema_rows.rowGroupStart boolean`（前端 schema 欄位，儲存於 Supabase `form_schemas` jsonb）標記「從此列起以下整組可重複新增」，搭配 AddFundsForm / EditFundsForm 的 `groupInstances` state 與 `__group_{blockId}` extra_data key 實作整組重複功能；付款憑單表單（`payment_voucher`）的付款明細區塊亦使用同一機制（建立頁 / 草稿編輯頁支援編輯，FundsPaymentDetail 以表格唯讀顯示）；`form_slots.defaultValue`（文字/選項預設值）與 `dateDefaultMode`（'fixed' | 'nearest_cycle'，僅 date 類型）：表單設定頁欄位設定面板可為任一欄位（除 readonly/attachment）設定新增申請單時自動帶入的預設值，date 類型可選「固定日期」或「自動選最近可用日期（依申請週期設定）」；目前僅 AddFundsForm（資金分配申請新增頁）讀取套用（僅全新表單生效，選了範本/草稿已有值時不覆蓋），付款憑單/暫付款沖銷憑單建立頁尚未串接。另外資金分配申請新增頁「申請處別」會依使用者組織架構身分自動帶入（`lib/orgPositions.ts` 的 `unitsInTreeOrder` 依組織樹順序取使用者第一個所屬處別，`nearestDivisionId` 往上找最近處別祖先；課別/職務不自動帶入，此為程式寫死邏輯，非表單設定頁可調整項目）；「職務」下拉選項文字改為「組織單位 + 職稱」（讀 `org_unit_members.role_type_id` 對照 `role_types.name`，無職稱則僅顯示單位名稱）；`form_slots.hint`（欄位說明小字，2026-07-16）：表單設定頁欄位設定面板可為任一欄位填一行說明，顯示在填寫頁欄位內容上方（純提示、不影響必填、不落地存），附件欄位用來列該階段常見應附單據，渲染共用 `RecordDetailView` 的 `FieldHint`／`DetailFieldLayout`（横式欄位有 hint 時標籤改頂端對齊），三個模組的新增/建立/草稿編輯頁皆已串接；提示內容存表單設定（form_schemas jsonb，dev/正式獨立，正式需自行同步）
...
src/
├── app/
│   ├── _components/       # 全局共享組件（SidebarLayout、ThemeProvider、HomeTabView、StatusBadge、WeekPicker、WeekFilterBar、ColumnPicker、useColumnVisibility、OrgScopeTree、SummaryCard、AllocationSummaryCard、ErrorDialog、ConfirmDialog、useConfirm、RecordDetailView、GroupEditTable、QuickReviewButtons 等）
...
│   │   └── all/           # 全部付款紀錄
│   ├── funds-voucher/
│   │   ├── _components/   # 沖銷專用共享組件（PaymentSummaryCard＝「預支的付款憑單」對照卡片、VoucherReturnSummary＝實際撥款/沖銷金額合計/回存金額彙總）
│   │   ├── my-voucher/    # 我的暫付款沖銷憑單列表
│   │   │   ├── [id]/      # 詳細頁（含送出審核）
│   │   │   └── add/[id]/  # 建立頁（由付款憑單 id 帶入）
│   │   ├── review/        # 審核管理
│   │   │   └── check/[id] # 審核操作頁
│   │   ├── share/[id]     # 分享連結中介轉址（依身份分流，接續列22）
│   │   └── all/           # 全部暫付款沖銷憑單紀錄
│   ├── finance/           # 財務管理
│   ├── system-settings/
...
│   ├── groupSlotVisibility.ts  # 付款明細群組欄位 showWhen 條件顯示判斷（isGroupSlotHidden/visibleGroupSlots，隱藏欄位計算與存檔視為 0，client/server 共用）
...
│   ├── attachmentSlots.ts      # 附件欄位與 fund_attachments.slot_label 對應規則（attachmentSlotLabels／firstAttachmentSlotLabel／attachmentsForSlot；對不上欄位的舊檔收進第一個附件欄位）
│   ├── paymentPrintEligibility.ts # 匯出付款憑單開放條件純函式（canExportPaymentVoucher：課處長關卡核准後即可匯出，詳細頁按鈕/列印頁防線/財務管理頁匯出欄三處共用）
│   ├── approvedItems.ts        # 逐項核准金額純函式（getAllocationGroupItems 解析申請單付款明細組／recomputeItemTax 稅額重算／sumApprovedItems 加總／latestApprovedItems 取最新一關，client/server 共用）
│   └── voucherReturnAmount.ts  # 暫付款沖銷「回存金額」純函式（paidAmountOf＝母憑單實際撥款、calcReturnAmount＝實撥−實際花費、voucherAmountOf＝沖銷單實際花費，client/server 共用）
└── proxy.ts               # 路由保護中間件
```
...
- **「剩餘金額」與「回存金額」是兩個不同的錢，絕對不可混用命名**：**剩餘金額**＝資金分配單還沒被憑單用掉的額度（核准金額 − 佔用，`lib/fundsAllocationRemaining.ts`），出現在資金分配三個列表、付款憑單建單頁對照卡片、財務管理頁等十幾處；**回存金額**＝預支的錢沒花完、同仁要還回公司的部分（母憑單實際撥款 − 沖銷單實際花費，`lib/voucherReturnAmount.ts`）。筑今系統把回存金額也叫「剩餘金額加總」，**我們刻意不跟**（2026-07-15 Yumin 拍板）——沖銷頁若再出現一個意思不同的「剩餘金額」，使用者與後續開發者都會被搞混。新增相關欄位/文案時務必確認自己講的是哪一個
- **母單對照卡片共用外框**：付款憑單頁的「已核准資金分配申請單」（`_components/AllocationSummaryCard.tsx`）與暫付款沖銷頁的「預支的付款憑單」（`funds-voucher/_components/PaymentSummaryCard.tsx`）共用 `_components/SummaryCard.tsx`（標題＋欄位列＋虛線＋金額列，`SummaryAmount` 為金額項、`danger` 顯示紅字）——**要改卡片顏色/間距/版面改 SummaryCard 一次連動**，勿在兩邊各自硬寫
...
- **群組明細「編輯」共用元件**：付款明細等群組區塊（`rowGroupStart`）的**編輯**畫面統一走 `src/app/_components/GroupEditTable.tsx`（2026-07-16 表格化，對齊筑今與唯讀頁的 `GroupDetailTable`）——表頭一次（label＋必填星號＋hint）、每組一列、行尾 ✕ 刪除、表格下方「＋新增項目」；**欄位順序＝表單設定群組列的攤平順序**（列欄配置對群組區塊只決定順序不決定排版，可在表單設定頁拖動調整、不用改程式）；欄寬依欄位型別自動配置（數字 125（容納七位數金額）／日期 130／下拉依該欄**最長選項文字**自動 110～280（呼叫端傳 `selectOptionLabels` 回呼提供選項，未提供時 200）／文字欄吃剩餘寬度），總寬超過容器時區塊內橫向捲動；元件是純渲染外殼，儲存格內容由各頁 `renderCell` 提供（稅額公式自動帶入、總額唯讀加總、費用項目主細項連動等計算邏輯留在各頁不動）。套用處共六個：資金分配 AddFundsForm/EditFundsForm（EditFundsForm 非 canEdit 時不傳 onAdd/onRemove＝唯讀）、付款憑單建立頁/草稿編輯頁、暫付款沖銷建立頁、表單設定範本分頁群組預填編輯器（去必填星號）。**要改群組編輯排版改這裡一次連動**。注意：表格容器是 `overflow-x: auto`，儲存格內的 `SearchableSelect` 必須開 `portal` 參數（下拉 createPortal 到 body、fixed 定位），否則下拉清單會被容器裁切——`portal` 預設 false、其他頁面行為不變
- **審核區塊共用元件**：三個審核操作頁（`funds-allocation`／`funds-payment`／`funds-voucher` 的 `review/check/[id]`）的「審核進度」動作區塊，統一由 `src/app/_components/ReviewProgressBlock.tsx` 提供，**比照筑今系統一列式排版**——每一個審核階段都是同一組欄位的橫向一列（階段名｜不核准○核准○｜評論(＋付款分類)｜核准金額｜確定送出），已完成階段顯示已選(反灰)結果＋審核人/時間、未來階段整列反灰空白、僅進行中階段（`canReview`）可操作。以 props 切換能力：`showApprovedAmount`（資金分配／付款憑單顯示核准金額欄，暫付款沖銷不顯示）、`enablePaymentCategory`（付款憑單／暫付款沖銷的「審核群組」步驟該列多一個付款分類下拉，資金分配不顯示）。radio 排序比照筑今為「不核准／核准」。各頁仍保留自己的送出/驗證邏輯（`handleSubmit`、approvedAmountCap、付款分類承接等），只把 controlled state（decision/comment/approvedAmount/paymentCategory）與 setter 傳進元件。**要改審核操作區塊的排版一次連動改這裡**。申請人自行查看審核進度也走這個元件的 `readOnly` 模式（三個申請人檢視頁一致：資金分配編輯頁 `EditFundsForm` 的審核進度面板、付款憑單詳細頁 `funds-payment/my-payment/[id]`、暫付款沖銷詳細頁 `funds-voucher/my-voucher/[id]`——後兩頁原本是純文字「審核歷程」列表，2026-07-15 改為共用此元件）：整列唯讀採四個固定欄位對齊——階段名｜不核准/核准 radio（寬 220 置中顯示該關結果）｜審核人＋審核人評論（填滿、靠左，格式「審核人名　評論：內容」，保留換行；審核者不核准/核准時留的評論即在此顯示給申請人看，2026-07-15 補上，此前 readOnly 完全沒畫評論欄導致申請人看不到退回理由）｜核准金額（寬 120 靠右，付款憑單／資金分配 `showApprovedAmount`，暫付款沖銷不顯示）；無可編輯評論框、無送出鈕（比照筑今申請人檢視）；`readOnly` 頁自行查詢步驟（`approval_flow_steps` 需含 `id`/`reviewer_type`）、`approval_records` 與審核人名（依 `reviewer_id` 撈 `app_users`），草稿狀態不顯示。比照筑今：僅進行中階段名深色、其餘（已核准／未輪到）皆灰字，不顯示「待審核」字樣；評論框與核准金額欄同高（56px）、整列垂直置中
