...
| 模組 | 路徑 | 說明 |
|------|------|------|
| 資金分配申請 | `/funds-allocation` | 申請、編輯、動態審核流程（範本驅動）、審核管理頁、全部申請紀錄；預選範本（共用範本 Admin 管理、個人範本另存；選取範本 Modal 個人範本可改名／點「編輯」進表單頁（`?editTemplateId=`）修改欄位值與刪除，僅限擁有者）；附件上傳（PDF/JPG/PNG，表單設定驅動，支援 Modal 預覽）；付款明細支援整組重複新增（`rowGroupStart` 標記起始列），稅額雙向自動計算，右上角彙總費用/手續費/稅額/總額；**審核人可在審核頁直接編輯申請單**，每次儲存記錄欄位級變更歷程（含附件新增/刪除、group block 逐行比對），可開 Modal 查看；**申請人視角顯示動態審核進度**（依範本步驟顯示已核准/不核准/待審核，含審核人評論與該步驟核准金額）；審核頁核准時可填寫/修改核准金額（存入 `approval_records.approved_amount`），已核准步驟旁會顯示「核准金額：X 元」；**審核管理 Tab 分類**：課、處長審核（依組織架構過濾）、諮詢議會、主管議會、財務長（依審核群組過濾，顯示該階段所有申請單）、我的審核紀錄；各 Tab 依出款帳戶分區塊顯示；帳號管理的系統角色可獨立開關每個 Tab 的可見權限（`fa-review-div` / `fa-review-advisory` / `fa-review-executive` / `fa-review-cfo`）；**週次篩選**：各 Tab 右上角自訂週次下拉選單（年份 + 週次，均為自訂樣式非 native select），依申請日期（`date` 欄位）過濾顯示該週所有申請單（不限狀態），已審核單子停留在對應 Tab；Tab 數字 badge 只計算目前仍在此步驟待審核的筆數；表格欄位「職稱」改名「職務」、「金額」改名「申請金額」，新增「核准金額」「剩餘金額」佔位欄（邏輯待補）；**列表欄位統一**：首頁「我的申請紀錄」、`/funds-allocation/my-funds`、`/funds-allocation/all` 三個列表頁欄位統一為審核管理頁的欄位組合（狀態/單號/申請處別/申請課別/申請人/職務/申請金額/核准金額/剩餘金額/出款帳戶/費用項目/項目），共用 `useColumnVisibility` hook + `ColumnPicker` 元件與 `lib/fundsAllocationColumns.ts` 欄位定義，可自由開關欄位（localStorage 記憶）；首頁與 my-funds 列表無「檢視 / 編輯」動作欄，由單號欄連結進入 |
| 付款憑單 | `/funds-payment` | 新增、動態審核流程（共用同一套範本架構）、審核管理頁、全部付款紀錄；**審核管理 Tab 分類**：課、處長審核（依組織架構過濾）＋動態群組 Tab（依啟用中付款憑單範本實際使用的審核群組產生）＋我的審核紀錄，各 Tab 依出款帳戶分區塊、右上角週次篩選（依 `date` 過濾該週全部狀態），權限 ID `fp-review-div` / `fp-review-group` 控制 Tab 可見性（帳號管理勾選）；繼承申請單附件（唯讀）＋可補傳本憑單附件；**付款明細與申請單同步**：付款明細區塊欄位與申請單相同（憑證類型/費用項目/幣別＋會計科目，群組列：稅額選擇/摘要/費用/手續費/稅額），建立時自動帶入申請單所有群組明細（多組全帶、以欄位 label 對應），草稿編輯頁支援逐組增刪修改（每組稅額自動計算），詳細/審核頁以表格顯示所有組並在標題列彙總費用/手續費/稅額/總額；憑單群組資料存 `extra_data.__group_{blockId}`，舊憑單無此 key 時退回申請單合併進來的 `__group_` 資料顯示（**注意：正式機 Supabase 為獨立專案，部署 main 後需在正式站表單設定同步調整付款憑單付款明細區塊欄位**）；**審核人解析**：憑單資料表沒有 `apply_division_id`/`apply_section_id` 欄位，範本步驟為課長/處長（org_role + section/division）時一律回溯關聯申請單的欄位（共用函式 `getAllocationOrgContext`，待審清單查詢用巢狀 embed），暫付款沖銷憑單再多回溯一層（temp_voucher → funds_payment → funds_allocation）；出款帳戶→範本查詢不可用 `maybeSingle()`（多筆綁定會靜默失敗使 `flow_template_id` 存成 null），統一走 `findPaymentVoucherTemplateId` / `findTempVoucherTemplateId`（`.limit(1)`） |
...
src/
├── app/
│   ├── _components/       # 全局共享組件（SidebarLayout、ThemeProvider、HomeTabView、StatusBadge、WeekPicker、ColumnPicker、useColumnVisibility 等）
