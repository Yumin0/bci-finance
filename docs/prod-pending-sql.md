...
- [ ] 正式機表單設定尚未同步付款明細區塊結構（資金分配重排＋付款憑單摘要/稅額選擇互換，見下）
- [x] ~~正式機舊申請單/範本 單據種類/幣別 頂層回填 SQL~~ **確認無需執行**（2026-07-16 Yumin 於正式機查證：`funds_allocation.extra_data` 與 `funds_allocation_templates.field_values` 皆無任何 `__group_%` key——正式站尚無群組格式的舊資料，沒有東西可回填；既有 4 筆頂層單據種類／12 筆頂層幣別為更早期表單直接存頂層的資料，格式本來就正確。下方回填 SQL 保留供日後參考，若正式站在表單同步前已產生群組格式單據才需要跑）
