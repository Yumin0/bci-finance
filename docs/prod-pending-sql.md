...
### Q6 國內公司隱藏三欄：資金分配付款明細 showWhen 設定（表單設定，非 SQL）（feature/yumin-q6-domestic-company-hide-tax）

- [ ] 正式站表單設定手動同步：資金分配表單付款明細群組的「手續費」「稅額選擇」「稅額」三個欄位，各設定條件顯示（showWhen）＝觸發欄位「單據種類」、勾選 國內個人／國外公司／國外個人（即選「國內公司」或未選時整欄隱藏，隱藏欄位計算與存檔一律視為 0）

操作路徑：系統設定 → 表單設定 → 資金分配 → 逐一點選「手續費」「稅額選擇」「稅額」→ 欄位設定面板的條件顯示選「單據種類」→ 勾 國內個人／國外公司／國外個人 → 儲存。
*為什麼：付款明細群組欄位的條件顯示是 2026-07-20 新接上的（`lib/groupSlotVisibility.ts`），行為由表單設定驅動、程式沒寫死欄位；dev/staging 已設定，正式機 Supabase 獨立、表單設定不會自動同步，未設定前正式機三欄照舊全部顯示（功能不會壞，只是 Q6 需求不生效）。*
