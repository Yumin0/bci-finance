...
### 正式站費用項目資料建置＋表單設定補細項欄位（費用類型設定＋表單設定）

- [x] **資料建置（2026-07-20，SQL）**：正式機新建「費用項目（細項）」類別（含 項次／項目名稱 兩欄位）＋複製 dev 全部 84 筆細項＋補建正式機獨有的 `2.3 業務銷售費用`、`6.3 入門服務費用` 兩筆細項（正式機主要清單比 dev 多這兩個編號）＋類別 id=1「費用項目」改名「費用項目（主要）」；配對檢查通過：32 個主要編號每個都有至少一筆對應細項
- [x] **表單設定（2026-07-20，Yumin 於正式站 UI 操作）**：付款明細群組內新增「費用項目（細項）」欄位（下拉、必填、資料來源＝費用項目（細項）（費用類型設定）），「整組重複新起始列」標在細項列（對齊 dev）；同時刪除群組內多餘的舊「費用項目」欄位（手續費/稅額選擇/稅額的條件顯示綁的是「單據種類」、不受影響；正式站舊單群組內若存過「費用項目」值，明細表格不再顯示該欄，資料仍在 extra_data 未消失）
- [x] **正式站實測通過（2026-07-20）**：新增申請單選主要「1.4 辦公室用品」→ 細項只列 1.4 開頭選項；兩組明細（1.4 文具用品 100／1.4 清潔費 200、摘要自動帶入項目值）送出後審核頁還原正確、費用/總額彙總 300

備註：正式站群組外「費用項目（主要）」欄位代號維持原有 `custom_1782788231927`（dev 為 `custom_expense_item_main`，兩者同為 `custom_` 自訂代號、存值機制相同，實測存檔還原正常；改代號需連帶掃 showWhen，維持現狀即可）。用途背景：主要/細項連動與「唯一細項自動帶入」（feature/riku-fee-detail-autofill）仰賴每個主要編號底下至少一筆細項，編號規則見 `lib/feeItems.ts`。

### Q6 國內公司隱藏三欄：資金分配付款明細 showWhen 設定（表單設定，非 SQL）（feature/yumin-q6-domestic-company-hide-tax）

- [x] **已執行（2026-07-20，Yumin 於正式站表單設定 UI 操作）**：資金分配表單付款明細群組的「手續費」「稅額選擇」「稅額」三個欄位，各設定條件顯示（showWhen）＝觸發欄位「單據種類」、勾選 國內個人／國外公司／國外個人（選「國內公司」或未選時整欄隱藏，隱藏欄位計算與存檔一律視為 0；`lib/groupSlotVisibility.ts`）
