...
// 欄寬抓「8 欄的資金分配付款明細在 13～14 吋筆電（側欄展開）不用橫向捲動」為基準；
// 空間有餘時文字欄（摘要）自動吃掉剩餘寬度，數字/下拉維持固定窄欄
function fixedWidthFor(slot: NonNullable<FormSlot>): number | undefined {
  switch (slot.type) {
    case 'number': return 95
    case 'date': return 130
    case 'select':
    case 'radio': return 220 // 選項多為中文詞組或英文名（如「1.3 Google Workspace」），太窄會截斷已選值
...
export default function GroupEditTable({
  ...
  const minTableWidth =
    slots.reduce((sum, s) => sum + (fixedWidthFor(s) ?? FLEX_MIN_WIDTH), 0) + (removable ? REMOVE_COL_WIDTH : 0)
