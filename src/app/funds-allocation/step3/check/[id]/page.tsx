...
export default function Step3CheckPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canReview = record?.status === FUNDS_STATUS.PENDING_STEP3
  ...
  async function handleSubmit() {
    ...
    const newStatus = decision === 'approved' ? FUNDS_STATUS.PENDING_STEP4 : FUNDS_STATUS.REJECTED_STEP3
    ...
    router.push('/funds-allocation/step3')
  }
  ...
  return (
    <div>
      ...
      {!canReview && !record.step3_decision && (
        <p style={{ color: 'gray', marginTop: 12 }}>此申請單目前狀態為「{record.status}」，不在諮詢議會審核階段。</p>
      )}
