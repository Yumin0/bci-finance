...
export default async function Step3ListPage() {
  const { data, error } = await supabase
    .from('funds_allocation')
    .select('*')
    .eq('status', FUNDS_STATUS.PENDING_STEP3)
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700 }}>諮詢議會單據管理列表</h1>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>待審核的資金分配申請單（狀態：{FUNDS_STATUS.PENDING_STEP3}）</p>
      </div>
          ...
          <tbody>
            ...
            {records.map((r) => (
              <tr key={r.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                ...
                <td style={td}>
                  <Link
                    href={`/funds-allocation/step3/check/${r.id}`}
