...
export default function Step4CheckPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canReview = record?.status === FUNDS_STATUS.PENDING_STEP4
  ...
  async function handleSubmit() {
    ...
    const newStatus = decision === 'approved' ? FUNDS_STATUS.PENDING_STEP5 : FUNDS_STATUS.REJECTED_STEP4
    ...
    router.push('/funds-allocation/step4')
  }
  ...
  return (
    <div>
      ...
      {!canReview && !record.step4_decision && (
        <p style={{ color: 'gray', marginTop: 12 }}>此申請單目前狀態為「{record.status}」，不在主管議會審核階段。</p>
      )}
