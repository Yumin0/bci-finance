...
// 「狀態」欄各頁自行處理（因 stepName 來源不同，且沖銷狀態標籤也疊在狀態欄裡顯示，比照列35a 資金分配的做法，
// 不另開一欄——避免表格多一欄把其他欄擠到自動換行、也不會讓「狀態」訊息被拆成兩個不相干的欄位），
// 此元件負責狀態之後的 8 欄。發票憑證欄已移除（2026-07-20，外層列表不看附檔、只留內層詳細頁附件欄位）。
