...
export default function PaymentStep2CheckPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canReview = record?.status === PAYMENT_STATUS.PENDING_STEP2
  ...
  async function handleSubmit() {
    ...
    const newStatus = decision === 'approved' ? PAYMENT_STATUS.PENDING_STEP3 : PAYMENT_STATUS.REJECTED_STEP2
    ...
    router.push('/funds-payment/step2')
  }
  ...
  return (
    <div>
      ...
      {!canReview && !record.step2_decision && (
        <p style={{ color: 'gray', marginTop: 12 }}>此付款憑單目前狀態為「{record.status}」，不在處級審核階段。</p>
      )}
