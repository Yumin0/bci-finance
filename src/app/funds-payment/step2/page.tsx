  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>待審核的付款憑單（狀態：{PAYMENT_STATUS.PENDING_STEP2}）</p>
      </div>
          ...
          <tbody>
            ...
            {records.map((r) => (
              <tr key={r.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                ...
                <td style={td}>
                  <Link href={`/funds-payment/step2/check/${r.id}`} className={buttonVariants({ variant: 'outline', size: 'sm' })}>檢視 / 審核</Link>
                </td>
              </tr>
            ))}
          </tbody>
