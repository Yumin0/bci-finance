...
export default function PaymentStep3CheckPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canReview = record?.status === PAYMENT_STATUS.PENDING_STEP3
  ...
  async function handleSubmit() {
    ...
    const newStatus = decision === 'approved' ? PAYMENT_STATUS.PENDING_STEP4 : PAYMENT_STATUS.REJECTED_STEP3
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        step3_comment: comment || null,
        step3_reviewer: MOCK_USER_ID,
        step3_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      })
      .eq('id', record.id)

    if (updateError) { setError(updateError.message); setSubmitting(false); return }
    router.push('/funds-payment/step3')
  }
  ...
  return (
    <div>
      ...
      {!canReview && !record.step3_decision && (
        <p style={{ color: 'gray', marginTop: 12 }}>此付款憑單目前狀態為「{record.status}」，不在第三處支出課審核階段。</p>
      )}
