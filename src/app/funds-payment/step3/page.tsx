...
export default async function PaymentStep3Page() {
  const { data, error } = await supabase
    .from('funds_payment')
    .select('*')
    .eq('status', PAYMENT_STATUS.PENDING_STEP3)
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700 }}>第三處支出課</h1>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>待審核的付款憑單（狀態：{PAYMENT_STATUS.PENDING_STEP3}）</p>
      </div>
          ...
          <tbody>
            ...
            {records.map((r) => (
              <tr key={r.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                ...
                <td style={td}>
                  <Link href={`/funds-payment/step3/check/${r.id}`} className={buttonVariants({ variant: 'outline', size: 'sm' })}>檢視 / 審核</Link>
                </td>
              </tr>
            ))}
          </tbody>
