...
export default function PaymentStep4CheckPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canReview = record?.status === PAYMENT_STATUS.PENDING_STEP4
  ...
  async function handleSubmit() {
    ...
    const newStatus = decision === 'approved' ? PAYMENT_STATUS.APPROVED : PAYMENT_STATUS.REJECTED_STEP4
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        step4_comment: comment || null,
        step4_reviewer: MOCK_USER_ID,
        step4_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      })
      .eq('id', record.id)

    if (updateError) { setError(updateError.message); setSubmitting(false); return }
    router.push('/funds-payment/step4')
  }
  ...
  return (
    <div>
      ...
      {!canReview && !record.step4_decision && (
        <p style={{ color: 'gray', marginTop: 12 }}>此付款憑單目前狀態為「{record.status}」，不在第三處處長審核階段。</p>
      )}
