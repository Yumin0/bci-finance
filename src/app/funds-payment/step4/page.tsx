...
export default async function PaymentStep4Page() {
  const { data, error } = await supabase
    .from('funds_payment')
    .select('*')
    .eq('status', PAYMENT_STATUS.PENDING_STEP4)
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700 }}>第三處 處長</h1>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>待審核的付款憑單（狀態：{PAYMENT_STATUS.PENDING_STEP4}）</p>
      </div>
