...
import { getTempVoucherForExport, type TempVoucherExportRow } from '@/app/actions/export'
import { formatDate } from '@/lib/dateUtils'
import { formatStatusLabel, type StatusLabelConfig } from '@/lib/status-label-config'

function resolveStepLabel(row: TempVoucherExportRow, labelConfig: StatusLabelConfig): string {
  const modConfig = labelConfig.temp_voucher
...
function buildColumns(labelConfig: StatusLabelConfig) {
  return [
    ...
    { key: 'amount',        label: '暫付金額', getValue: (r: TempVoucherExportRow) => r.amount != null ? r.amount : '' },
    { key: 'flow',          label: '審核流程', getValue: (r: TempVoucherExportRow) => r.approval_flow_templates?.name ?? '' },
    { key: 'progress',      label: '狀態',     getValue: (r: TempVoucherExportRow) => resolveStepLabel(r, labelConfig) },
  ]
}

function downloadCsv(
  rows: TempVoucherExportRow[],
  ...
  a.href = url
  a.download = `temp_voucher_${new Date().toISOString().slice(0, 10)}.csv`
...
export default function ExportCsvButton({ labelConfig }: { labelConfig: StatusLabelConfig }) {
  const COLUMNS = buildColumns(labelConfig)
  const tv = labelConfig.temp_voucher
  ...
  async function handleExport() {
    ...
    const { data, error: fetchError } = await getTempVoucherForExport({ dateFrom, dateTo, applicant, status })
    setLoading(false)
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                ...
                >
                  ...
                  <option value="pending">{tv['pending']?.label ?? '審核中'}</option>
                  <option value="approved">{tv['approved']?.label ?? '已核准'}</option>
                  <option value="rejected">{tv['rejected']?.label ?? '未核准'}</option>
                </select>
              </div>
