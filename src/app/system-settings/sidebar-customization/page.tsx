...
export default function SidebarCustomizationPage() {
  const [config, setConfig] = useState<SidebarCategory[]>(() => clone(DEFAULT_SIDEBAR_CONFIG))
  const [saved, setSaved] = useState(false)
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(new Set(['admin-permissions']))

  useEffect(() => {
    try {
      const stored = localStorage.getItem(SIDEBAR_STORAGE_KEY)
      if (stored) {
        const parsed: SidebarCategory[] = JSON.parse(stored)
        setConfig(parsed)
        const ids = new Set<string>()
        parsed.forEach(cat => cat.entries.forEach(e => { if (e.kind === 'group') ids.add(e.id) }))
  ...
  function moveCategory(ci: number, dir: -1 | 1) {
    update(c => {
      const t = ci + dir
      if (t >= 0 && t < c.length) [c[ci], c[t]] = [c[t], c[ci]]
    })
  }
  ...
  function moveEntry(ci: number, ei: number, dir: -1 | 1) {
    update(c => {
      const t = ei + dir
      if (t >= 0 && t < c[ci].entries.length) [c[ci].entries[ei], c[ci].entries[t]] = [c[ci].entries[t], c[ci].entries[ei]]
    })
  }

  function moveGroupItem(ci: number, ei: number, ii: number, dir: -1 | 1) {
    update(c => {
      const items = (c[ci].entries[ei] as SidebarGroup).items
      const t = ii + dir
      if (t >= 0 && t < items.length) [items[ii], items[t]] = [items[t], items[ii]]
    })
  }
  ...
  function dissolveGroup(ci: number, ei: number) {
    update(c => {
      const group = c[ci].entries[ei] as SidebarGroup
      c[ci].entries.splice(ei, 1, ...group.items)
    })
  }
    ...
    <div style={{ maxWidth: 740 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={handleReset} style={outlineBtn}>還原預設</button>
          <button onClick={handleSave} style={solidBtn}>{saved ? '已儲存' : '儲存設定'}</button>
        </div>
      </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#f1f5f9', padding: '12px 16px', borderBottom: '1px solid #e5e7eb' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <input
                  ...
                  onChange={e => update(c => { c[ci].label = e.target.value })}
                  style={inp}
                />
              </div>
            </div>
                ...
                return (
                  <div key={entry.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', background: '#fff', borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <input
                        value={entry.label}
                        onChange={e => update(c => { (c[ci].entries[ei] as SidebarItem).label = e.target.value })}
                        style={{ ...inp, fontSize: 13 }}
                      />
                    </div>
                    <span style={{ fontSize: 12, color: '#d1d5db', fontFamily: 'monospace', whiteSpace: 'nowrap', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 220 }}>{entry.href}</span>
                    {groups.length > 0 && (
                      <select
                        value=""
                        onChange={e => { if (e.target.value) moveItemIntoGroup(ci, ei, e.target.value) }}
                        style={{ fontSize: 12, padding: '4px 6px', border: '1px solid #d1d5db', borderRadius: 4, color: '#374151', cursor: 'pointer', flexShrink: 0 }}
                      >
                ...
                <div key={group.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', background: '#fafafa' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <input
                        value={group.label}
                        onChange={e => update(c => { (c[ci].entries[ei] as SidebarGroup).label = e.target.value })}
                        style={{ ...inp, fontSize: 13 }}
                      />
                    </div>
                  ...
                  {open && (
                    <>
                      {group.items.map((item, ii) => (
                        <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 16px 9px 52px', background: '#fff', borderTop: '1px solid #f3f4f6' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <input
                              value={item.label}
                              onChange={e => update(c => { (c[ci].entries[ei] as SidebarGroup).items[ii].label = e.target.value })}
                              style={{ ...inp, fontSize: 13 }}
                            />
                          </div>
                      ...
                      {group.items.length === 0 && (
                        <div style={{ padding: '10px 52px', fontSize: 13, color: '#9ca3af', borderTop: '1px solid #f3f4f6' }}>尚無項目，可從上方平層項目選擇「加入群組」</div>
                      )}
                    </>
                  )}
                </div>
      ...
      {saved && <p style={{ fontSize: 13, color: '#16a34a', marginTop: 4 }}>設定已儲存，側邊欄已即時更新。</p>}
    </div>
  )
}
...
const inp: React.CSSProperties = {
  width: '100%', padding: '6px 10px', border: '1px solid #d1d5db', borderRadius: 5,
  fontSize: 14, color: '#111827', outline: 'none', boxSizing: 'border-box',
}
const solidBtn: React.CSSProperties = {
  padding: '8px 20px', background: '#111827', color: '#fff', border: 'none',
...
function arrowBtn(disabled: boolean): React.CSSProperties {
  return {
    ...
    width: 22, height: 18, fontSize: 9, lineHeight: 1, padding: 0,
    background: disabled ? '#f9fafb' : '#fff', border: '1px solid #e5e7eb',
